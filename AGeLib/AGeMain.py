...
try:
    from AGeLib.QtVersion import UsePyQt5
except ModuleNotFoundError:
    from QtVersion import UsePyQt5
#try:
...
def trap_exc_during_debug(*args):
    ...
    print("An unhandled exception occurred in a QThread!!!")
    print(args)
    print()
    NC(1,"An unhandled exception occurred in a QThread!!!",exc=args)
...
class NC: # Notification Class
    ...
    """
    def __init__(self, lvl=None, msg=None, time=None, input=None, err=None, tb=None, exc=None, win=None, func=None, DplStr=None, log=True, send=True,processEvents=True):
        """
 ...
 #
    ...
    def __init__(self, args):
        ...
        try:
            ...
            self.ModuleVersions = "Python %s\nAGeLib %s\nSymPy %s\nNumpy %s\nMatplotLib %s\nPySide2 Qt %s" % ("%d.%d" % (sys.version_info.major, sys.version_info.minor),
                    version,
                    sympy.__version__,
                    numpy_version,
                    matplotlib.__version__,
...
class StackedWidget(QtWidgets.QStackedWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.LButton.pressed.connect(self.L)
        ...
        self.RButton.pressed.connect(self.R)
...
class MplWidget_LaTeX(MplWidget):
    def __init__(self, parent=None):
        ...
        self.ToggleButton.pressed.connect(self.Toggle)
        ...
        self.StackedWidget.currentChanged.connect(self._ModeSwitched)
    ...
    def DisplayRaw(self, Text_L, Text_N, Font_Size = None, Use_LaTeX = False):
        ...
        if self.StackedWidget.currentIndex() == 0:
            ...
            pageSource = r"""
                        ...
                        <style type="text/css">
                        ::-webkit-scrollbar {width: 6px; height: 4px; background: """+str(App().Palette1.base().color().name(0))+r"""; }
                        ::-webkit-scrollbar-thumb { background-color: """+str(App().Palette1.alternateBase().color().name(0))+r"""; -webkit-border-radius: 1ex; }
                        </style>
                        <body bgcolor=" """+str(App().Palette1.base().color().name(0))+r""" ">
                        <p><mathjax style="font-size:2.3em; color: """+str(App().palette().text().color().name(0))+\
            ...
            return NC(4,"Mode {} is not implemented".format(str(self.StackedWidget.currentIndex())),input="Mode: {}".format(str(self.StackedWidget.currentIndex())),win=self.window().windowTitle(),func=str(self.objectName())+".DisplayRaw",send=False)
...
class OptionsWidget_1_Appearance(QtWidgets.QWidget):
    ...
    def SavePalette(self,Name=None):
        ...
        Text += "if sUsePyQt5: from PyQt5 import QtCore, QtGui\nelse: from PySide2 import QtCore, QtGui"
        Text += "\n\ndef NewColour():\n    palette1 = QtGui.QPalette()\n    palette2 = QtGui.QPalette()\n    palette3 = QtGui.QPalette()"
        for i in self.PaletteColours:
...
class exec_Window(AWWF):
    ...
    def execute_code(self):
        ...
        try:
            ...
            mw = window # pylint: disable=unused-variable
            #TODO: Make it possible to add the globals from the applications main script to allow the use of its functions and imports
            exec(input_text)
        except common_exceptions:
